...
| TrenchBroom level loading | 🟡 | `.map` parser → **general brush geometry** (plane∩plane∩plane, angled brushes) → per-face polygons + **convex-hull collision** → entity dispatch → `buildWorld(mapPath)`; showcase kept as fallback. **Brush-entity movers/triggers verified end-to-end from a real `.map`** (`map_brush_entities` scenario: `func_door` blocks→opens on `trigger_multiple`, `trigger_hurt` damages) — box-approximation path. Pending: full Quake-UV texture fidelity, on-demand texture loading, hot-reload, true brush-hull fidelity for non-cuboid brush entities, retiring the C++ showcase |
